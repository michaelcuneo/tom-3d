...
function decodeJwtPayload<T extends Record<string, unknown>>(jwt: string | null): T | null {
	if (!jwt) return null;
...
export async function POST(event) {
	...
	interface JwtPayload {
		exp?: number;
		[i: string]: unknown; // Add additional properties if needed
	}

	const claims: JwtPayload | null = decodeJwtPayload<JwtPayload>(access);
	...
	try {
		...
		const updatedClaims: JwtPayload | null = decodeJwtPayload<JwtPayload>(currentAccess);

<script lang="ts">
	...
	let paperOverlay: HTMLDivElement;

	let cursorX: string = $state('50vh');
	let cursorY: string = $state('50vw');
	let cursorX2: string = $state('50vw');
	let cursorY2: string = $state('50vh');

	const updateAnimationTiming = (e: MouseEvent) => {
		cursorX = e.clientX + 'px';
		cursorY = e.clientY + 'px';
		cursorX2 = e.clientX - ((12 * w) / 1000) * (e.clientX / w - 0.5) + 'px';
		cursorY2 = e.clientY - ((12 * h) / 1000) * (e.clientY / h - 0.5) + 'px';
	};
</script>

<svelte:window bind:innerHeight={h} bind:innerWidth={w} onmousemove={updateAnimationTiming} />

<div
	style="--cursorX: {cursorX}; --cursorY: {cursorY}; --cursorX2: {cursorX2}; --cursorY2: {cursorY2};"
	class="paperOverlay"
	bind:this={paperOverlay}
></div>
...
<style>
	...
	.paperOverlay:before {
		...
		animation: torch 1s linear 2s infinite alternate;
	}
